...
def make_test_command(*modules, **kwargs):
    ...
    def test(coverage=None, verbose=None, loglevel='WARN'):
        ...
        # tested.
        import unittest
        from ki.shared.testing import runner as runner_cls

        # Set up logging
        level = getattr(logging, loglevel.upper(), logging.WARN)
        logging.getLogger().setLevel(level)

        # Figure out how to instantiate our test runner. The verbosity arg
        # only applies to the built-in TextTestRunner.
        if runner_cls == unittest.TextTestRunner:
            verbosity = 1 if verbose is None else 2
            runner = runner_cls(verbosity=verbosity)
        else:
            runner = runner_cls()
